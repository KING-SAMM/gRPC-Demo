...
type GreetServiceClient interface {
	...
	BiDirectionalStreaming(ctx context.Context, opts ...grpc.CallOption) (GreetService_BiDirectionalStreamingClient, error)
}
...
func (c *greetServiceClient) BiDirectionalStreaming(ctx context.Context, opts ...grpc.CallOption) (GreetService_BiDirectionalStreamingClient, error) {
	stream, err := c.cc.NewStream(ctx, &GreetService_ServiceDesc.Streams[2], "/greet_service.GreetService/BiDirectionalStreaming", opts...)
	if err != nil {
		return nil, err
	}
	x := &greetServiceBiDirectionalStreamingClient{stream}
	return x, nil
}

type GreetService_BiDirectionalStreamingClient interface {
	Send(*HelloRequest) error
	Recv() (*HelloResponse, error)
	grpc.ClientStream
}

type greetServiceBiDirectionalStreamingClient struct {
	grpc.ClientStream
}

func (x *greetServiceBiDirectionalStreamingClient) Send(m *HelloRequest) error {
	return x.ClientStream.SendMsg(m)
}

func (x *greetServiceBiDirectionalStreamingClient) Recv() (*HelloResponse, error) {
	m := new(HelloResponse)
	if err := x.ClientStream.RecvMsg(m); err != nil {
		return nil, err
	}
...
type GreetServiceServer interface {
	...
	// Bidirectional streaming
	BiDirectionalStreaming(GreetService_BiDirectionalStreamingServer) error
...
func (UnimplementedGreetServiceServer) BiDirectionalStreaming(GreetService_BiDirectionalStreamingServer) error {
...
func _GreetService_BiDirectionalStreaming_Handler(srv interface{}, stream grpc.ServerStream) error {
	return srv.(GreetServiceServer).BiDirectionalStreaming(&greetServiceBiDirectionalStreamingServer{stream})
}

type GreetService_BiDirectionalStreamingServer interface {
	Send(*HelloResponse) error
	Recv() (*HelloRequest, error)
	grpc.ServerStream
}

type greetServiceBiDirectionalStreamingServer struct {
	grpc.ServerStream
}

func (x *greetServiceBiDirectionalStreamingServer) Send(m *HelloResponse) error {
	return x.ServerStream.SendMsg(m)
}

func (x *greetServiceBiDirectionalStreamingServer) Recv() (*HelloRequest, error) {
	m := new(HelloRequest)
	if err := x.ServerStream.RecvMsg(m); err != nil {
		return nil, err
	}
...
var GreetService_ServiceDesc = grpc.ServiceDesc{
	...
	Streams: []grpc.StreamDesc{
		...
		},
		{
			...
			Handler:       _GreetService_BiDirectionalStreaming_Handler,
			ServerStreams: true,
			ClientStreams: true,
		},
	},
